...
bool Level3::OnEnter(const GameServices& services) {
	...
	const float columnCount = 12.f, rowCount = 6.f, gap = 10.f;
	// ========================
	...
	m_greenBrick = m_redBrick;
	m_greenBrick.color = D3DCOLOR_XRGB(0, 200, 0);
	...
	// add Green Brick to top center
	const int greenIdx = 6;                 
	m_isGreen[greenIdx] = true;
	m_greenHP[greenIdx] = 100;
	m_isBlue[greenIdx] = false;            // make sure it isn’t treated as blue
	m_bricksList[greenIdx].color = m_greenBrick.color;

	// Load Bg
	m_background.textureHandle = services.renderer.LoadTexture("assets/bg4.jpg");
...
void Level3::OnExit(const GameServices& services) {
	services.soundManager.StopAll();
}

void Level3::Update(float dt, InputManager& inputManager, PhysicsManager& physicsManager, SoundManager& soundManager) {
	// Change Game State
	  // Lose
	if (life <= 0) {
	...
	// --- 5) Ball vs Bricks using swept AABB to prevent tunneling ---
	{
		...
		while (remaining > 0.0f && hits < maxHits) {
			...
			if (hitIndex >= 0) {
				...
				// Handle green/blue/regular
				if (m_isGreen[hitIndex]) {
					// GREEN: needs 100 hits total
					if (m_greenHP[hitIndex] > 0) {
						--m_greenHP[hitIndex];
						soundManager.Play("hard_hit"); // feedback per hit
						// Optionally flash color by HP %
						// e.g., brighten as it gets lower:
						 int t = 255 - (m_greenHP[hitIndex] * 2); // crude mapping
						 m_bricksList[hitIndex].color = D3DCOLOR_XRGB(0, std::clamp(t, 80, 255), 0);
						if (m_greenHP[hitIndex] <= 0 && m_bricksList[hitIndex].visible) {
							m_bricksList[hitIndex].visible = false;
							--m_aliveBricks;
						}
					}
				} else {
					// BLUE spawns balls (you already do that above), but removal is normal one-hit
					if (m_bricksList[hitIndex].visible) {
						m_bricksList[hitIndex].visible = false;
						--m_aliveBricks;
						soundManager.Play("hit");
					}
	...
	for (auto& bb : m_blueBalls) {
		...
		while (remainingBB > 0.0f && bbHits < maxBBHits) {
			...
			//soundManager.Play("hit");
			if (m_isGreen[hitIdx]) {
				if (m_greenHP[hitIdx] > 0) {
					--m_greenHP[hitIdx];
					soundManager.Play("hard_hit");
					if (m_greenHP[hitIdx] <= 0 && m_bricksList[hitIdx].visible) {
						m_bricksList[hitIdx].visible = false;
						--m_aliveBricks;
					}
				}
			} else {
				if (m_bricksList[hitIdx].visible) {
					m_bricksList[hitIdx].visible = false;
					--m_aliveBricks;
					soundManager.Play("hit");
				}
...
void Level3::Render(Renderer& renderer) {
	...
	//for (int i = 0; i < m_brickCount; ++i) {
	//	if (m_isGreen[i] && m_bricksList[i].visible) {
	//		std::wstringstream gs;
	//		gs << L"HP: " << m_greenHP[i];
	//		renderer.DrawTextString(gs.str(),
	//			(int)(m_bricksList[i].position.x - 20),
	//			(int)(m_bricksList[i].position.y - 10),
	//			D3DCOLOR_XRGB(0,0,0));
	//	}
	//}
	for (int i = 0; i < m_brickCount; ++i) {
		if (m_isGreen[i] && m_bricksList[i].visible) {
			...
			const float bx = m_bricksList[i].position.x;
			const float by = m_bricksList[i].position.y;

			// Pick a spot inside the brick (slightly above center)
...
void Level3::SpawnBlueBallAt(const D3DXVECTOR3& pos) {
	BlueBall bb{};
