...
bool Level2::OnEnter(const GameServices& services) {
	...
	const float columnCount = 12.f, rowCount = 5.f, gap = 10.f;
	// ========================
	...
	// Load Bg
	m_background.textureHandle = services.renderer.LoadTexture("assets/bg3.png");
...
void Level2::OnExit(const GameServices& services) {
	services.soundManager.StopAll();
}

void Level2::Update(float dt, InputManager& inputManager, PhysicsManager& physicsManager, SoundManager& soundManager) {
	// Change Game State
	  // Lose
	if (life <= 0) {
...
void Level2::SpawnBlueBallAt(const D3DXVECTOR3& pos) {
	BlueBall bb{};
